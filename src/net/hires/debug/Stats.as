...
package net.hires.debug {
	...
	public class Stats extends Sprite {	
		...
		public function Stats() : void {
			...
			xml = <xml><fps>FPS:</fps><fmem>MS:</fmem><mem>MEM:</mem><memMax>MAX:</memMax></xml>;
		
			style = new StyleSheet();
			style.setStyle('xml', {fontSize:'9px', fontFamily:'_sans', leading:'-2px'});
			style.setStyle('fps', {color: hex2css(colors.fps)});
			style.setStyle('fmem', {color: hex2css(colors.fmem)});
		...
		private function update(e : Event) : void {
			...
			if( timer - 1000 > ms_prev ) {
				...
				ms_prev = timer;
				mem = Number((System.totalMemoryNumber * 0.000000954).toFixed(3));
				...
				graph.setPixel(graph.width - 1, graph.height - ( ( timer - ms ) >> 1 ), colors.fmem);
				...
				xml.fmem = "FMEM: " + Number((System.freeMemory * 0.000000954).toFixed(3));
				xml.mem = "MEM: " + mem;
